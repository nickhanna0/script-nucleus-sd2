...
### 为什么需要 Bible
- 任何“确认过”的设定都必须被固化，否则对话越长越容易漂移。
- 后续围绕 CLIP 的修改讨论，最终稿必须回写 Bible，再由 Bible 下发到分镜/提示词。
...
- 第二次确认（分镜确认）后：写入该集 episodeId、对应 storyboardId、关键变更摘要
...
### 第一次确认：策略确认
- 通过后才生成分镜
- **确认后必须调用 QC 做“逻辑框架检查”**（不看镜头细节）
- 确认通过后回写 Bible

### 第二次确认：分镜确认
- 通过后才进入 PROMOTE
- **确认后必须调用 QC 做“冲突/漂移/可生成性”检查**
- 确认通过后回写 Bible
